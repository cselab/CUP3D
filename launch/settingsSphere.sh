...
NU=${NU:-0.000015625}
BC=${BC:-dirichlet}
FACTORY='Sphere L=0.125 xpos=0.6 xvel=0.125 bForcedInSimFrame=1 bFixFrameOfRef_x=1 bBreakSymmetry=1'

OPTIONS=
OPTIONS+=" -bpdx ${BPDX} -bpdy ${BPDY} -bpdz ${BPDZ}"
OPTIONS+=" -dump2D 0 -dump3D 1 -tdump 10 -tend 100 "
OPTIONS+=" -BC_x ${BC} -BC_y ${BC} -BC_z ${BC}"
OPTIONS+=" -CFL ${CFL} -lambda ${LAMBDA} -use-dlm ${DLM} -nu ${NU}"
OPTIONS+=" -ImplicitPenalization 1"
OPTIONS+=" -levelMax ${LEVELS} -levelStart 3 -Rtol 0.40 -Ctol 0.10"
OPTIONS+=" -extentx 4.0 "
OPTIONS+=" -TimeOrder 2 "
OPTIONS+=" -poissonTol ${PT} "
OPTIONS+=" -poissonTolRel ${PTR} "
