...
for i in np.arange(numFish):
  for j in np.arange(numFish):
    for k in np.arange(numFish):
      ...
      # ADD FISH ON EDGE
      f.write('FACTORY+="CarlingFish L=0.2 T=1.0 xpos={} ypos={} zpos={} bFixToPlanar=1 bFixFrameOfRef=1 heightProfile=stefan widthProfile=stefan\n\
...
OPTIONS+=" -dump2D 0 -dump3D 1 -tdump 0.25 -tend 10.0 "\n\
