...
BASENAME=FallingSamaraFixed_long_1212_.001Inertia_

cd ../makefiles/;make clean;make config=production bc=mixed precision=single particles=false dlm=true hdf=true movingframe=true -j;cd ../launch/
...
for LAMBDA in 1
do
for ISO in 0.0 0.004 0.008 0.012
do
OPTIONS=" -bpdx ${B} -bpdy $[4*${B}] -bpdz ${B} -CFL ${CFL} -shape samara -rhoS 250. -fdump 250 -lambda ${LAMBDA} -dlm ${LAMBDA} -nu 0.00001 -isosurface ${ISO}"
