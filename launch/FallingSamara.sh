...
BASENAME=FallingSamara_v2_0812_

cd ../makefiles/;make clean;make config=production bc=mixed precision=single particles=false dlm=true hdf=true -j;cd ../launch/
...
do
	OPTIONS=" -bpdx ${B} -bpdy $[4*${B}] -bpdz ${B} -CFL ${CFL} -shape samara -rhoS 250. -fdump 250 -lambda ${LAMBDA} -dlm ${LAMBDA} -nu 0.000042"

	NAME=DLM${LAMBDA}_CFL${CFL}_$[${B}*32]
