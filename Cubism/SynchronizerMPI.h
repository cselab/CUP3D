	...
	virtual void sync(unsigned int gptfloats, MPI::Datatype MPIREAL, const int timestamp)
	{
		...
		//0.
		{
			...
			if (NPENDINGSENDS > 0)
			{
				...
				while (1)
				{
					done = MPI::Request::Testall(NPENDINGSENDS, &pending.front());
					if (done) break;
					sched_yield();
				};
#endif
...
#pragma omp parallel for schedule(runtime)
	...
	//peh
	virtual void sync0(unsigned int gptfloats, MPI::Datatype MPIREAL, const int timestamp)
	{
		double t0, t1;
...
#pragma omp parallel for //schedule(runtime)
	...
	vector<BlockInfo> avail_halo()
	{
		...
		while (1)
		{
			done = MPI::Request::Testall(NPENDING, &pending.front());
			if (done) break;
			pthread_yield();
		};
#endif
	...
	bool test_halo()
	{
		...
		bool done = false;
		done = MPI::Request::Testall(NPENDING, &pending.front(), status);
