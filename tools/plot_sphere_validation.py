...
def plotValidation():
  ...
  for i in range( len(cases) ):
    ...
    axs[i].plot(time[::10], totDrag[::10], color=colors[i], label="Present method")
    axs[i].set_xlabel("Time")# $T=tu/D$")
    axs[i].set_ylim([0.5*totDrag[-1],2*totDrag[-1]])
    axs[i].legend(bbox_to_anchor=(0,-0.25, 1, 1), loc="lower left",
                mode="expand", borderaxespad=0, ncol=2, frameon=False)

  axs[0].set_ylabel("Drag Coefficient")# $C_D=2|F_x|/\pi r^2u^2$")
