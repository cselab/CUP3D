...
namespace FakeShader {
	...
	void shader_main(float rho, RGBA& color)
	{
		...
		vec3 cazzo = min(vec3(.6), normalize(vec3(.2,0,1.))*redmodulate) + min(vec3(0.2*redmodulate), vec3(cos_value + 0.5*pow(max(0.f,cos_value),10.f)));		
		for(int i=0; i<3; ++i) color[i] += cazzo.data[i]*factor;
		
		// green
		float greenmodulate = modulate_lighting_effect_green(rho);
		cazzo = 0;//min(vec3(.1), normalize(vec3(1.0,1.0,1.0))*greenmodulate) + min(vec3(.01*greenmodulate), vec3(cos_value + 0.5*pow(max(0.f,cos_value),10.f)));		
		for(int i=0; i<3; ++i) color[i] += 10*cazzo.data[i]*factor;
		
		// blue - purple
		float bluemodulate = modulate_lighting_effect_blue(rho);
		cazzo = min(vec3(.6), normalize(vec3(.1,.2,1.))*bluemodulate) + min(vec3(1.*bluemodulate), vec3(cos_value + 0.5*pow(max(0.f,cos_value),10.f)));		
		for(int i=0; i<3; ++i) color[i] += cazzo.data[i]*factor;
