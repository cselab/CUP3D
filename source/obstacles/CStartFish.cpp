...
class ControlledCurvatureDefinedFishData : public FishMidlineData
{
 public:
 ...
 public:
  ...
  ControlledCurvatureDefinedFishData(double L, double T, double phi, double _h, const double _ampFac)
  ...
  ~ControlledCurvatureDefinedFishData() override
  {
...
void ControlledCurvatureDefinedFishData::execute(const double time, const double l_tnext,
...
void ControlledCurvatureDefinedFishData::computeMidline(const double t, const double dt)
{
...
{
  auto * const cFish = dynamic_cast<ControlledCurvatureDefinedFishData*>( myFish );
...
CStartFish::CStartFish(SimulationData & s, ArgumentParser&p) : Fish(s, p)
{
  ...
  myFish = new ControlledCurvatureDefinedFishData(length, Tperiod, phaseShift,
    sim.maxH(), ampFac);
...
#if 0
void CStartFish::execute(const int iAgent, const double time, const std::vector<double> act)
{
