...
protected:
  ...
  void _cub2fftw(TGrid& grid, Real * out, const size_t nx, const size_t ny,
  ...
  virtual void _solve(mycomplex * in_out, const size_t nx, const size_t ny,
    const size_t nz, const size_t nz_hat, const double norm_factor, const double h)
  {
    const Real h2 = h*h;
    ...
    {
      const size_t linidx = (i*ny+j)*nz_hat + k;
  ...
  virtual void _solveSpectral(mycomplex * in_out, const size_t nx, const size_t ny,
    const size_t nz, const size_t nz_hat, const double norm_factor, const double h)
  {
    const double waveFactX = 2*M_PI/(nx*double(h));
    const double waveFactY = 2*M_PI/(ny*double(h));
    const double waveFactZ = 2*M_PI/(nz*double(h));
    #pragma omp parallel for
    for(int i=0; i<nx; ++i)
    for(int j = 0; j<ny; ++j)
    for(int k = 0; k<nz_hat; ++k)
    {
      ...
      const double rkx = kx*waveFactX;
      const double rky = ky*waveFactY;
      const double rkz = kz*waveFactZ;
      const double kinv = (kx==0 && ky==0 && kz==0) ? 0.0 : -1.0/(rkx*rkx+rky*rky+rkz*rkz);
  ...
  void _fftw2cub(Real * out, TGrid& grid, const size_t nx, const size_t ny,
                      const size_t nz, const size_t nz_hat, const double dt) const
  {
    vector<BlockInfo> infos = grid.getBlocksInfo();
    const size_t N = infos.size();
...
public:
  ...
  void solve(TGrid& grid, const double dt, const bool spectral=false)
  {
    const int bs[3] = {BlockType::sizeX, BlockType::sizeY, BlockType::sizeZ};
    const size_t gsize[3] = {grid.getBlocksPerDimension(0)*bs[0], grid.getBlocksPerDimension(1)*bs[1], grid.getBlocksPerDimension(2)*bs[2]};
...
public:
  ...
  void solve(TGrid& grid, const Real dt, const bool spectral=false)
  {
    // dimensions of blocks and whole grid
    const int bs[3] = {BlockType::sizeX, BlockType::sizeY, BlockType::sizeZ};
    const size_t gsize[3] = {grid.getBlocksPerDimension(0)*bs[0], grid.getBlocksPerDimension(1)*bs[1], grid.getBlocksPerDimension(2)*bs[2]};
    ...
    profiler.push_start("FFTW2CUB");
    _fftw2cub(data, grid, gsize[0], gsize[1], gsize[2], dt);
    profiler.pop_stop();
