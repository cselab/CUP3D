...
void _fourier_filter_kernel(Complex*const __restrict__ out,
  const int*const N, const int*const osize, const int*const ostart,
  const Real*const waveFact, const Real fac)
{
...
void _fourier_filter_gpu(Complex*const __restrict__ out, const int N[3],
  const int osize[3], const int ostart[3], const double h)
{
  ...
  dim3 Db(4, 4, 4);
  _fourier_filter_kernel<<<Dg, Db>>>(out, n_gpu, osize_gpu, ostart_gpu, fac_gpu, scale);
