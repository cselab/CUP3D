/*
 *  SerializerIO_WaveletCompression_Simple.h
...
class SerializerIO_WaveletCompression_SimpleBlocking
{
protected:
...
#pragma omp for
	...
	virtual void _to_file(const MPI::Intracomm& mycomm, const string fileName)
	{
		...
		myfileinfo.Set("access_style", "write_once");	// peh: fixed typo in first argument
...
#else
			myfile.Write_at(current_displacement + myfileoffset, &allmydata.front(), written_bytes, MPI_CHAR);
#endif
...
#else
			myfile.Write_at(current_displacement + mygid * metadata_bytes, &myblockindices.front(), metadata_bytes, MPI_CHAR);
#endif			
...
#else
			myfile.Write_at(current_displacement + mygid * lutheader_bytes, &lutheader, lutheader_bytes, MPI_CHAR);
#endif
		}
...
public:
	...
	SerializerIO_WaveletCompression_SimpleBlocking():
...
template<typename GridType, typename IterativeStreamer>
class SerializerIO_WaveletCompression_Simple : public SerializerIO_WaveletCompression_SimpleBlocking<GridType, IterativeStreamer>
{	
	size_t nofcalls;
	...
	void _to_file(const MPI::Intracomm& mycomm, const string fileName)	
	{
		...
		myfileinfo.Set("access_style", "write_once");	// peh: fixed typo
...
#else
			myopenfile.write(&current_displacement, sizeof(current_displacement), MPI_CHAR) ;
			myopenfile.write(this->binaryocean_title.c_str(), this->binaryocean_title.size(), MPI_CHAR) ;				
#endif
		}
...
#else
				myopenfile.write_at(current_displacement, this->header.c_str(), header_bytes, MPI_CHAR);
#endif
...
#else
			myopenfile.write_at(current_displacement + mygid * metadata_bytes, &this->myblockindices.front(), metadata_bytes, MPI_CHAR);
#endif			
			current_displacement += metadata_bytes * nranks;			
		}
...
#else
				myopenfile.write_at(current_displacement, this->binarylut_title.c_str(), title_bytes, MPI_CHAR);				
#endif
...
#else
			myopenfile.write_at(current_displacement + mygid * lutheader_bytes, &this->lutheader, lutheader_bytes, MPI_CHAR) ;
#endif
		}
...
public:
	SerializerIO_WaveletCompression_Simple():
	SerializerIO_WaveletCompression_SimpleBlocking<GridType, IterativeStreamer>(),
	nofcalls(0)
	{
	}
