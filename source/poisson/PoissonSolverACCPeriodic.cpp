...
PoissonSolverPeriodic::PoissonSolverPeriodic(SimulationData & s) : PoissonSolver(s)
{
  ...
  acc_plan* P = accfft_plan_dft(_gsize, rho_gpu,phi_hat, c_comm,ACCFFT_MEASURE);
  plan = (void*) P;
}
...
void PoissonSolverPeriodic::solve()
{
  ...
  MPI_Barrier(c_comm);
  // Perform forward FFT
  accfft_exec_r2c((acc_plan*)plan, rho_gpu, (acc_c*)phi_hat);
  // Spectral solve
  MPI_Barrier(c_comm);
  _fourier_filter_gpu((acc_c*)phi_hat, gsize, osize, ostart, h);
  // Perform backward FFT
  MPI_Barrier(c_comm);
  accfft_exec_c2r((acc_plan*)plan, (acc_c*)phi_hat, rho_gpu);
  MPI_Barrier(c_comm);
