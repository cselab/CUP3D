...
class My3DFFT_Infinite_MPI
{
 ...
 private:
  ...
  // domain dimensions
  size_t m_N0;  // Nx-points of original domain
  size_t m_N1;  // Ny-points of original domain
  size_t m_N2;  // Nz-points of original domain
  size_t m_NN0; // Nx-points of padded domain
  size_t m_NN1; // Ny-points of padded domain
  size_t m_NN2; // Nz-points of padded domain
  size_t m_NN0t; //Nx of padded domain (w/o periodic copies, actual transform size)
  size_t m_NN1t; //Ny of padded domain (w/o periodic copies, actual transform size)
  size_t m_NN2t; //Nz of padded domain (w/o periodic copies, actual transform size)
  Real m_h; // uniform grid spacing

  // MPI related
  MPI_Comm m_comm;
  int m_rank, m_size;
  size_t m_local_N0, m_start_N0;
  size_t m_local_NN0, m_start_NN0, m_local_NN1, m_start_NN1;
  ...
  size_t m_Nzhat; // for r2c transform
  size_t m_tp_size;
  size_t m_full_size;
  ...
  void _set_kernel(Real* const kern)
  {
      ...
      // mirror
      // octant 100
      const size_t vol = m_local_NN0 * m_N1 * m_N2;
      ...
      if (m_rank < m_size/2)
      {
          ...
          for (size_t k = 0; k < m_N2; ++k)
          {
              const size_t src = k + 2*m_Nzhat*(j + m_NN1*i);
              const size_t dst = k + m_N2*(j + m_N1*(m_local_NN0-1-i));
      ...
      {
          size_t src_rank = m_size - m_rank - 1;
          MPI_Recv(buf, vol, MPIREAL, src_rank, src_rank, m_comm, MPI_STATUSES_IGNORE);
          for (size_t i = 0; i < m_local_NN0; ++i)
          for (size_t j = 0; j < m_N1; ++j)
          for (size_t k = 0; k < m_N2; ++k)
          {
              const size_t dst = k + 2*m_Nzhat*(j + m_NN1*i);
              const size_t src = k + m_N2*(j + m_N1*i);
              kern[dst] = buf[src];
          }
      }
      delete[] buf;
      ...
      for (size_t k = 0; k < m_N2; ++k)
      {
          const size_t src = k + 2*m_Nzhat*((m_N1-1-j) + m_NN1*i);
          const size_t dst = k + 2*m_Nzhat*((m_N1+j) + m_NN1*i);
          kern[dst] = kern[src];
      }
      ...
      for (size_t k = 0; k < m_N2; ++k)
      {
          const size_t src = m_N2-1-k + 2*m_Nzhat*((m_N1-1-j) + m_NN1*i);
          const size_t dst = m_N2+k + 2*m_Nzhat*((m_N1+j) + m_NN1*i);
          kern[dst] = kern[src];
      }
      ...
      for (size_t k = 0; k < m_N2; ++k)
      {
          const size_t src = m_N2-1-k + 2*m_Nzhat*(j + m_NN1*i);
          const size_t dst = m_N2+k + 2*m_Nzhat*(j + m_NN1*i);
          kern[dst] = kern[src];
      }
  }
};
...
{
 public:
  PoissonSolverUnbounded(SimulationData & s) : sim(s)
  {
      const size_t N0  = m_grid.getBlocksPerDimension(0)*BlockType::sizeX;
      const size_t N1  = m_grid.getBlocksPerDimension(1)*BlockType::sizeY;
      const size_t N2  = m_grid.getBlocksPerDimension(2)*BlockType::sizeZ;
  ...
  ~PoissonSolverUnbounded() = default;
  ...
  void _cub2fftw()
  {
    const size_t NlocBlocks = m_local_infos.size();
    #pragma omp parallel for
 ...
 private:
  ...
  // computational grid
  TGrid& m_grid = * sim.grid;

  // local grid blocks
  const std::vector<BlockInfo> m_local_infos = m_grid.getResidentBlocksInfo();
