...
//  CubismUP_3D
//
//  Written by Guido Novati and Fabian Wermelinger ( novatig@ethz.ch ).
...
class PoissonSolverScalarFFTW_MPI
{
  ...
  const size_t gsize_0[3] = {
      2*gsize[0], // x-size for zero-padded arrays.  Best if gsize[0] is a power of two.
      2*gsize[1], // y-size for zero-padded arrays.  Best if gsize[1] is a power of two.
      2*gsize[2]  // z-size for zero-padded arrays.  Best if gsize[2] is a power of two.
  };
