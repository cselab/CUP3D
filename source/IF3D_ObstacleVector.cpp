...
struct rayTracing
{
    ...
    Real findDistanceOnSkin(const Real* const xSkin, const Real* const ySkin, const int N,
    						const int jPOV, const int pRay) const
    {
    ...
    void execute(const Real lengthscale)
    {
      ...
      for (int j=0; j<FOVobst.size(); j++)  //loop over obstacles
          ...
          if(p>2*FOVdata[j]->NpLatLine-1) continue;
...
void IF3D_ObstacleVector::getFieldOfView(const Real lengthscale)
{
  #ifdef __useSkin_
