...
#ifdef _USE_HDF_
	stringstream ss;
	ss << path2file << "-ic" << ic << "-bpd" << bpd << "-IC";
	cout << ss.str() << endl;
	DumpHDF5_MPI<FluidGridMPI, StreamerHDF5>(*grid, 0, ss.str());
#endif
}

TestVarCoeffPoisson::TestVarCoeffPoisson(const int argc, const char ** argv, const int ic, const int bpd) : Test(argc, argv, bpd), ic(ic)
{
	// output settings
