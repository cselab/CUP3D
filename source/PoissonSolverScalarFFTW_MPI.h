...
protected:
																		...
																		const size_t nz, const size_t nz_hat) const
	{
		...
		for(int i=0; i<N; ++i) {
			...
			const size_t offset = bs[2]*info.index[2] + nz_hat * 2 * (
...
public:
	...
	PoissonSolverScalarFFTW_MPI(const int desired_threads, TGrid& grid):
	FFTWBase_MPI(desired_threads), initialized(false)
	{
