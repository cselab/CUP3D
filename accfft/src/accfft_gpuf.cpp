    ...
    float * data_out_d, MPI_Comm c_comm, unsigned flags)
{
  ...
  if (plan->oneD)
  {
    plan->Mem_mgr = new Mem_Mgr_gpu<float>(n[0], n[1], n_tuples_o, c_comm, 1, plan->alloc_max);
    plan->T_plan_2 = new T_Plan_gpu<float>(n[0], n[1], n_tuples_o,
        plan->Mem_mgr, c_comm);
    plan->T_plan_2i = new T_Plan_gpu<float>(n[1], n[0], n_tuples_o,
        plan->Mem_mgr, c_comm);
    plan->T_plan_1 = NULL;
    plan->T_plan_1i = NULL;
  ...
  if (direction == -1)
  {
    ...
    if (xyz[2])
    {
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
        ...
        {
          checkCuda_accfft(
              cufftExecR2C(plan->fplan_0,
                  (cufftReal*) &data_d[ i * osize_0[1]  * n_tuples_i ],
                  (cufftComplex*) &data_out_d[ i * osize_0[1] * n_tuples_o ]));
        }
      #else
        checkCuda_accfft(
    ...
    if (xyz[1])
    {
      ...
      for (size_t i = 0; i < (size_t) osize_1[0]; ++i) {
        checkCuda_accfft( cufftExecC2C(plan->fplan_1,
            (cufftComplex*) &data_out_d[2 * i * osize_1[1] * osize_1[2]],
            (cufftComplex*) &data_out_d[2 * i * osize_1[1] * osize_1[2]],
            CUFFT_FORWARD));
      }
      checkCuda_accfft(cudaEventRecord(fft_stopEvent, 0));
    ...
    if (xyz[0])
    {
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
        ...
        for (size_t i = 0; i < (size_t) osize_2[1]; ++i) {
          checkCuda_accfft(
              cufftExecC2C(plan->fplan_2,
                (cufftComplex*) &data_out_d[2 *i *osize_2[2]],
                (cufftComplex*) &data_out_d[2 *i *osize_2[2]], CUFFT_FORWARD));
        }
      #else
        checkCuda_accfft(
  ...
  else if (direction == 1)
  {
    if (xyz[0])
    {
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
        ...
        for (size_t i = 0; i < (size_t) osize_2[1]; ++i) {
          checkCuda_accfft(
              cufftExecC2C(plan->fplan_2,
                (cufftComplex*) &data_d[2 *i *osize_2[2]],
                (cufftComplex*) &data_d[2 *i *osize_2[2]], CUFFT_INVERSE));
      }
      #else
        checkCuda_accfft(
    ...
    if (xyz[1])
    {
      ...
      for (size_t i = 0; i < (size_t) osize_1i[0]; ++i) {
        checkCuda_accfft( cufftExecC2C(plan->fplan_1,
                (cufftComplex*) &data_d[2 * i * n[1] * osize_1i[2]],
                (cufftComplex*) &data_d[2 * i * n[1] * osize_1i[2]],
                CUFFT_INVERSE));
      }
      checkCuda_accfft(cudaEventRecord(fft_stopEvent, 0));
    ...
    if (xyz[2])
    {
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
        ...
        int n_tuples_o = (n[2] / 2 + 1) * 2;
        for (size_t i = 0; i < (size_t) osize_0[0]; ++i) {
          checkCuda_accfft( cufftExecC2R(plan->iplan_0,
                (cufftComplex*) &data_d[ i * osize_0[1] * n_tuples_o ],
                (cufftReal*) &data_out_d[ i * osize_0[1]  * n_tuples_i ]));
        }
      #else
        checkCuda_accfft(
...
void accfft_execute_y_gpuf(accfft_plan_gpuf* plan, int direction, float * data_d,
  ...
  float* cwork_d = nullptr; //plan->Mem_mgr->buffer_d3;
...
void accfft_execute_x_gpuf(accfft_plan_gpuf* plan, int direction, float * data_d,
  ...
  if (direction == -1) {
    ...
    /*******************  N0/P0 x N1 x N2/P1 **********************/
    /**************************************************************/
      printf("DOING fplan_x\n"); fflush(0); abort();
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
      for (size_t i = 0; i < (size_t) osize_x[1]; ++i) {
        checkCuda_accfft(
            cufftExecR2C(plan->fplan_x,
                (cufftReal*) &cwork_d[i *osize_x[2]],
                (cufftComplex*) &data_out_d[2 *i *osize_xi[2]] ) );
      }
      //checkCuda_accfft(
    ...
    /*******************  N0/P0 x N1 x N2/P1 **********************/
    /**************************************************************/
    printf("DOING iplan_x\n"); fflush(0); abort();
      checkCuda_accfft(cudaEventRecord(fft_startEvent, 0));
      for (size_t i = 0; i < (size_t) osize_x[1]; ++i) {
        checkCuda_accfft(
            cufftExecC2R(plan->iplan_x,
                (cufftComplex*) &data_d[2 *i *osize_xi[2]],
                (cufftReal*) &data_d[i *osize_x[2]] ) );
      }
      //checkCuda_accfft(
